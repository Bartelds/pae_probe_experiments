...
def main():
    ...
    parser.add_argument("-i", "--input_dir", default="./data/timit/wav/", type=Path)
    parser.add_argument("-o", "--output_dir", default="../feats/{model}/layer-{layer}/")
    parser.add_argument("-n", "--output_name", default="{speaker}.npy")
    parser.add_argument("-m", "--model", default="wav2vec2-base")
    parser.add_argument("-l", "--layer", default='all')
    ...
    for l in ls:
        ...
        # Load acoustic model
        featurizer = load_wav2vec2_featurizer(args.model, layer=int(l))

        # Create features for each wav file
        for wav_path in tqdm(wav_paths, ncols=80):
            # Check output directory
            speaker = os.path.splitext(wav_path.name)[0]
            output_dir = Path(args.output_dir.format(model=args.model, layer=int(l)))

            # Extract features
            hidden_states = featurizer(wav_path)
            if int(l) is not None:
                hidden_states = [hidden_states]

            # Save features
            os.makedirs(output_dir, exist_ok=True)
            for layer, hidden_state in enumerate(hidden_states, start=int(l) or 0):
                feat_path = output_dir / args.output_name.format(speaker=speaker)
                np.save(feat_path, hidden_state)

            tqdm.write(str(output_dir) + '/' + str(args.output_name.format(speaker=speaker)))
